...
def get_time(timezone: str) -> dict:
    try:
        ...
        return {
            "error": f"알 수 없는 시간대입니다: {timezone}",
            "suggestion": "예: Asia/Seoul, Asia/Tokyo, UTC",
        }
...
tools = [
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "timezone": {"type": "string", "description": "예: Asia/Seoul"}
            },
            "required": ["timezone"],
            "additionalProperties": False,
        },
...
final_answer, tool_trace = run_local_tool_call_loop(
    client=client,
    model=model,
    user_prompt="서울 날씨와 Invalid/Timezone 현재 시간을 알려줘. 시간대가 잘못되면 이유와 올바른 예시도 설명해줘.",
    tools=tools,
