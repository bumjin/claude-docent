...
from .openai_prompt_templates import history_based_prompt, tool_system_prompt
...
tools = [
    {
        ...
        "description": "사용자가 시대와 장르를 모두 명시해 전시물 검색을 요청한 경우에만 사용",
        "parameters": Category.model_json_schema(),
    },
    {
        "name": "search_historical_facts",
        "description": "역사적 사실과 배경 설명이 필요한 질문에 대해 보조 정보를 검색",
        "parameters": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "검색용 핵심 질의어"},
            },
            "required": ["query"],
...
def search_relics_by_period_and_genre(
    ...
    message = (
        f"요청하신 전시물이 {len(results)}건 검색되었습니다. [다음] 버튼을 눌러 확인해 주세요."
        if len(results) > 0
...
def use_tools(
    ...
    payload = openai_llm.choose_tool(
        messages=messages,
        tools=tools,
